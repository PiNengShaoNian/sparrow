...
// bool值取反
static bool primBoolNot(UNUSED VM *vm, Value *args)
{
...
static bool primThreadCurrent(VM *vm, UNUSED Value *args)
{
  RET_OBJ(vm->curThread);
}

// Thread.suspend():挂起线程,退出解析器
static bool primThreadSuspend(VM *vm, UNUSED Value *args)
{
  // 目前suspend操作只会退出虚拟机,
...
static bool primThreadYieldWithoutArg(VM *vm, UNUSED Value *args)
{
  ObjThread *curThread = vm->curThread;
  vm->curThread = curThread->caller; // 使cpu控制权回到主调方
...
static bool primThreadIsDone(UNUSED VM *vm, Value *args)
{
  // 获取.isDone的调用者
...
#define PRIM_NUM_BIT(name, operator)               \
  static bool name(UNUSED VM *vm, Value *args)     \
  {                                                \
    if (!validateNum(vm, args[1]))                 \
      return false;                                \
...
// 这里用fmod实现浮点取模
static bool primNumMod(UNUSED VM *vm, Value *args)
{
  if (!validateNum(vm, args[1]))
    return false;
...
// 数字取反
static bool primNumBitNot(UNUSED VM *vm, Value *args)
{
  RET_NUM(~(uint32_t)VALUE_TO_NUM(args[0]));
}

//[数字from..数字to]
static bool primNumRange(UNUSED VM *vm, Value *args)
{
  if (!validateNum(vm, args[1]))
    return false;
...
// 返回小数部分
static bool primNumFraction(UNUSED VM *vm, Value *args)
{
  double dummyInteger;
...
// 判断数字是否无穷大,不区分正负无穷大
static bool primNumIsInfinity(UNUSED VM *vm, Value *args)
{
  RET_BOOL(isinf(VALUE_TO_NUM(args[0])));
}

// 判断是否为数字
static bool primNumIsInteger(UNUSED VM *vm, Value *args)
{
...
// 取数字的整数部分
static bool primNumTruncate(UNUSED VM *vm, Value *args)
{
  double integer;
...
// 判断两个数字是否相等
static bool primNumEqual(UNUSED VM *vm, Value *args)
{
  if (!validateNum(vm, args[1]))
    RET_FALSE;
...
static bool primStringByteAt(UNUSED VM *vm, Value *args)
{
  ObjString *objString = VALUE_TO_OBJSTR(args[0]);
  uint32_t index = validateIndex(vm, args[1], objString->value.length);
  if (index == UINT32_MAX)
...
static bool primStringByteCount(UNUSED VM *vm, Value *args)
{
  RET_NUM(VALUE_TO_OBJSTR(args[0])->value.length);
}

// objString.codePointAt_(_):返回指定的CodePoint
static bool primStringCodePointAt(UNUSED VM *vm, Value *args)
{
  ObjString *objString = VALUE_TO_OBJSTR(args[0]);
  uint32_t index = validateIndex(vm, args[1], objString->value.length);
  if (index == UINT32_MAX)
...
static bool primStringContains(UNUSED VM *vm, Value *args)
{
  if (!validateString(vm, args[1]))
    return false;
...
static bool primStringEndsWith(UNUSED VM *vm, Value *args)
{
  if (!validateString(vm, args[1]))
    return false;
...
static bool primStringIndexOf(UNUSED VM *vm, Value *args)
{
  if (!validateString(vm, args[1]))
    return false;
...
static bool primStringIterate(UNUSED VM *vm, Value *args)
{
  ObjString *objString = VALUE_TO_OBJSTR(args[0]);
...
static bool primStringIterateByte(UNUSED VM *vm, Value *args)
{
  ObjString *objString = VALUE_TO_OBJSTR(args[0]);
...
static bool primStringStartsWith(UNUSED VM *vm, Value *args)
{
  if (!validateString(vm, args[1]))
    return false;
...
static bool primStringToString(UNUSED VM *vm, Value *args)
{
  RET_VALUE(args[0]);
}

// objList.new():创建1个新的list
static bool primListNew(VM *vm, UNUSED Value *args)
{
...
static bool primListSubscriptSetter(UNUSED VM *vm, Value *args)
{
  // 获取对象
  ObjList *objList = VALUE_TO_OBJLIST(args[0]);
...
static bool primListCount(UNUSED VM *vm, Value *args)
{
...
// objMap.new():创建map对象
static bool primMapNew(VM *vm, UNUSED Value *args)
{
...
static bool primMapCount(UNUSED VM *vm, Value *args)
{
...
static bool primRangeFrom(UNUSED VM *vm, Value *args)
{
...
static bool primRangeMin(UNUSED VM *vm, Value *args)
{
...
// 因此直接返回迭代器就是range的值
static bool primRangeIteratorValue(UNUSED VM *vm, Value *args)
{
  ObjRange *objRange = VALUE_TO_OBJRANGE(args[0]);
